---
name: ai-image-post
description: AI 生图全流程：用 ImageGen（混元生图）生成 → 去右下角"AI生成 WORKBUDDY"水印 → PIL 叠加中文标题（AI 渲染中文易崩，叠加最稳）→ 裁剪到目标尺寸（如 336x200 社区封面）→ 归档到 02-Areas-资产/九思-数字资产/设计资产。适用于：生成封面图、海报、配图；图片带水印需要清理；需要给图加中文标题；需要调整图片尺寸。触发词：封面图、生图、去水印、加标题、336x200、配图。
agent_created: true
---
...
### Step 5：归档
- 成品存 `02-Areas-资产/九思-数字资产/设计资产/`（命名：`主题-类型-尺寸.png`）
